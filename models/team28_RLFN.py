...
class RLFN_Prune(nn.Module):
    def __init__(self, in_nc=3, out_nc=3, nf=44, mf=44, upscale=4):
        super().__init__()

        self.conv_1 = conv_layer(in_nc, nf, kernel_size=3)

        self.block_1 = RLFB(in_channels=nf, mid_channel = mf)
        self.block_2 = RLFB(in_channels=nf, mid_channel = mf)
        self.block_3 = RLFB(in_channels=nf, mid_channel = mf)
        self.block_4 = RLFB(in_channels=nf, mid_channel = mf)

        self.conv_2 = conv_layer(nf, nf, kernel_size=3)
    ...
    def forward(self, input):
        out_fea = self.conv_1(input)
        
        out_B = self.block_1(out_fea)
        out_B = self.block_2(out_B)
        out_B = self.block_3(out_B)
        out_B = self.block_4(out_B)

        out_lr = self.conv_2(out_B) + out_fea
